...
public class GestorDeProductosJDBC {
	...
	public void guardarProductosEnLaBaseDeDatos(ArrayList<Producto> productosRecuperados) throws ProductoException {
		...
		try {
			...
			throw new ProductoException("****Error Al Guardar Los Productos En La Base De Datos****", e);
		} finally {
			if(connection != null) {
				try {
					connection.close();
	...
	public ArrayList<Producto> recuperarProductosDeLaDaseDeDatos() throws ProductoException {
		DBUtils dbUtils = new DBUtils();
		...
		try {
			...
			throw new ProductoException("****Error Al Intentar Recuperar Los Productos De La Base De Datos*****", e);
		} finally {
			if(connection != null) {
				try {
					connection.close();
